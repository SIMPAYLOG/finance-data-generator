...
import com.simpaylog.generatorapi.dto.DetailOccupationInfo;
...
public class DetailOccupationLocalCache {
    ...
    public Map<Integer, DetailOccupationInfo.SubOccupation> getSubOccupations(String code) {
        return cache.get(code).subOccupations();
    }

    public DetailOccupationInfo.SubOccupation getSubOccupationsByCodeAndDecile(String code, int decile) {
    ...
    public void init() {
        try {
            ...
            Map<Integer, DetailOccupationInfo.Job> map = new HashMap<>();
            for (String key : data.keySet()) {
                cache.put(key, data.get(key));
            }
            log.info(String.format("[DetailOccupationLocalCache] 캐시 로딩 완료: %d개", map.size()));
        } catch (Exception e) {
